...
class CosineLrScheduler(StatefulSchedulerBase):
    """
    Cosine annealing learning rate scheduler, as described in this paper https://arxiv.org/abs/1608.03983,
    but without restarts.

    This scheduler differs from the PyTorch's ``~torch.optim.lr_scheduler.CosineAnnealingLR`` as it provides options
    ...
    as described in this paper https://arxiv.org/abs/2004.05909.
    ...
    def __init__(
        ...
        :param lr_min: the minimum value that the learning rate should decay to for all parameter groups.
        This will be held fixed during cooldown epochs
        :param min_lr_ratio: this can be used to represent the minimum lr for each parameter group as a ratio of its maximum lr.
         If set, this will take precedence over `lr_min`
        :param num_warmup_epochs: the number of epochs to gradually increase the lr until it reaches the maximum value
        :param warmup_starting_lr: the starting lr that will be used for all parameter groups at the beginning of training
        if `num_warmup_epochs` is greater than 0
        :param warmup_starting_lr_ratio: this can be used to represent the warmup starting lr for each parameter group as a
        ratio of its maximum lr. If set, this will take precedence over `warmup_starting_lr`
    ...
    def get_updated_values(self, num_updates: int):
        ...
        if num_updates < self.warmup_iterations:
            ...
            lrs = [
                (self.warmup_lr_ratio * lr if self.warmup_lr_ratio is not None else self.warmup_lr_init)
                + num_updates * ((lr - self.warmup_lr_init) / self.warmup_iterations)
                for lr in self.base_lr_values
            ]
        elif num_updates < (
            self.total_iterations - (self.warmup_iterations + self.num_cooldown_iterations)
        ):
    ...
    def create_scheduler_fn(
        ...
        ratio of its maximum lr. If set, this will take precedence over `warmup_starting_lr`
        :param num_cooldown_epochs: the number of epochs to hold the lr at its minimum value
        :return: a function which accepts an optimizer as an argument and returns an instance of `CosineLrScheduler`
        """
        return partial(
            cls,
